...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    const displayNames = [];
    arr.forEach(prop => displayNames.push(`name: ${prop.animal_name}, scientific: ${prop.scientific_name}`));
    return displayNames;
  }
  ...
  */
  ...
  function lowerCaseNames(arr){
    let newArr = arr.map(prop => prop.animal_name.toLowerCase());
    return newArr;
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    let newArr = arr.filter(prop => prop.population < 5);
    return newArr;
  }
